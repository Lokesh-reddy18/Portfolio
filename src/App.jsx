  ...
  const socialLinks = [
    {
      ...
      icon: (
        <img
          src={`https://skillicons.dev/icons?i=github&theme=${darkMode ? 'dark' : 'light'}`}
          alt="GitHub"
          className="w-6 h-6"
        />
      )
    },
    {
      ...
      icon: (
        <img
          src={`https://skillicons.dev/icons?i=linkedin&theme=${darkMode ? 'dark' : 'light'}`}
          alt="LinkedIn"
          className="w-6 h-6"
        />
      )
    },
    {
      ...
      icon: (
        <img
          src={`https://skillicons.dev/icons?i=gmail&theme=${darkMode ? 'dark' : 'light'}`}
          alt="Gmail"
          className="w-6 h-6"
        />
      )
    },
    {
      ...
      icon: (
        <img
          src={`https://skillicons.dev/icons?i=instagram&theme=${darkMode ? 'dark' : 'light'}`}
          alt="Instagram"
          className="w-6 h-6"
        />
      )
          ...
          <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} p-8 rounded-lg shadow-md`}>
            <div className="flex flex-wrap justify-center gap-4">
              {skills.map((skill, idx) => (
                ...
                >
                  <img 
                    src={`https://skillicons.dev/icons?i=${skill.slug}&theme=${darkMode ? 'dark' : 'light'}`}
                    alt={skill.name}
                    className="w-12 h-12 mb-2 object-contain animate-bounce-slow"
                  />
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {projects.map((project, index) => (
              <div 
                key={index} 
                className={`relative group border ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'} rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-3 hover:scale-105`}
              ...
              >
                <div className="relative">
                  <img 
                    src={project.image} 
                    alt={project.title} 
                    className="w-full h-48 object-cover rounded-t-2xl border-b border-gray-200 dark:border-gray-700" 
                  />
                </div>
                ...
                <div className="p-6 flex flex-col h-full">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.technologies.map((tech, techIndex) => (
                      <span key={techIndex} className={`${darkMode ? 'bg-blue-950 text-blue-200 border-blue-800' : 'bg-blue-50 text-blue-800 border-blue-200'} border text-xs px-2 py-1 rounded-full font-medium`}>{tech}</span>
                    ))}
                  </div>
                  <div className="flex space-x-3 mt-4"> {/* Changed from mt-auto to mt-4 */}
                    <a 
                      href={project.demoUrl} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className={`flex-1 text-center py-2 px-3 rounded-lg font-semibold border transition-all duration-200 ${darkMode ? 'border-blue-400 text-blue-200 hover:bg-blue-900/60' : 'border-blue-600 text-blue-700 hover:bg-blue-50'}`}
                    >
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className={`flex-1 text-center py-2 px-3 rounded-lg font-semibold border transition-all duration-200 ${darkMode ? 'border-gray-500 text-gray-200 hover:bg-gray-700' : 'border-gray-400 text-gray-700 hover:bg-gray-100'}`}
                    >
                      <span className="inline-flex items-center justify-center gap-1">
